...
#define TEVS_PIXEL_RATE		 40000000// 61430400
...
static const struct v4l2_mbus_framefmt tevs_csi2_default_fmt = {
...
static int _tevs_set_routing(struct v4l2_subdev *sub_dev,
			       			 ...
			       			 struct v4l2_subdev_krouting *routing)
{
	...
	ret = v4l2_subdev_set_routing(sub_dev, state, routing);
	if (ret)
		return ret;
...
static int tevs_init_cfg(struct v4l2_subdev *sub_dev,
                          struct v4l2_subdev_state *state)
{
	struct v4l2_subdev_route routes[] = {
		{
			.source_pad = TEVS_PAD_SOURCE0,
			.source_stream = 0,
	...
	return _tevs_set_routing(sub_dev, state, &routing);
}
                                ...
                                struct v4l2_mbus_frame_desc *fd)
{
	...
	state = v4l2_subdev_get_locked_active_state(sub_dev);
...
static int tevs_set_routing(struct v4l2_subdev *sub_dev,
                             struct v4l2_subdev_state *state,
                             ...
                             struct v4l2_subdev_krouting *routing)
{
	...
	ret = _tevs_set_routing(sub_dev, state, routing);
...
static int tevs_set_pixel_rate(struct tevs *tevs, s32 value)
{
	return 0;
}
static int tevs_get_pixel_rate(struct tevs *tevs, s32 *value)
{
	return 0;
}
...
static int tevs_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_PIXEL_RATE:
		return tevs_set_pixel_rate(tevs, ctrl->val);
...
static int tevs_g_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_PIXEL_RATE:
		return tevs_get_pixel_rate(tevs, &ctrl->val);
...
static int tevs_probe(struct i2c_client *client)
{
	...
	sd = &tevs->v4l2_subdev;
	v4l2_i2c_subdev_init(sd, client,
			     &tevs_subdev_ops);
	...
	dev_dbg(dev, "register the subdev\n");
